...
デリバリーチームを本来注力すべき業務に集中させ、組織全体の重複作業を減らしたいという要望から、企業はクラウドネイティブコンピューティング向けのプラットフォームを導入するようになりました。プラットフォームに投資することで、企業は次のことが可能になります：
...
優れたプラットフォームは、Webポータル、プロジェクトテンプレート、セルフサービスAPIなどを通じて提供される機能やサービスについて、一貫性があり、設計方針の明確なユーザー体験を提供します。
...
マネージドサービスプロバイダーや社内の専門チームがバックエンドの実装を維持する一方、プラットフォームは、提供される実装全体に一貫性を与えつつ、組織要件を満たすための必要最小限のレイヤーです。
...
これには、多くの場合、個別に管理され、独自のビジネスコンポーネントと統合されるリソースや機能が必要となります。こうしたリソースには、データベースやオブジェクトストア、メッセージキューやブローカー、テレメトリー収集ツールやダッシュボード、ユーザーディレクトリや認証システム、タスクランナーやリコンサイラー(差分検出器)などが含まれます。
...
プラットフォームの成熟度に関するより詳細なビジョンについては、[プラットフォームエンジニアリング成熟度モデル](https://cloudnativeplatforms.com/ja/whitepapers/platform-eng-maturity-model/)を参照してください。
...
1. **ユーザーエクスペリエンス**。プラットフォームは、一貫性のあるインターフェースを通じてその能力を提供し、ユーザーエクスペリエンスを重視すべきです。プラットフォームは、ユーザーが人間であってもAIエージェントであっても利用しやすいよう、状況に応じたインターフェースを提供する必要があります。そのため、GUI、API、コマンドラインツール、IDE、ポータルを組み合わせて提供することが求められます。例えば、開発者はIDEを通じてその能力を利用し、テスターはコマンドラインツールを使用するのに対して、プロダクトオーナーはGUIベースのWebポータルを利用する場合があります。
...
1. **ユーザーの認知負荷を軽減**。プラットフォームの重要な目標は、プロダクトチームにおける認知負荷を軽減することです。プラットフォームは実装の詳細をカプセル化し、そのアーキテクチャから生じる可能性のあるあらゆる複雑性を隠蔽すべきです。例えば、プラットフォームは特定のサービスをクラウドプロバイダーに委託する場合がありますが、ユーザーはそうした詳細を一切知らされることはありません。同時に、プラットフォームは、必要に応じてユーザーが特定のサービスを設定・監視できるようにすべきです。一方で、プラットフォームが提供するサービスの運用をユーザーが担うべきではありません。例えば、ユーザーはデータベースを必要とすることがあっても、データベースサーバー自体を管理する必要はありません。
...
1. **デフォルトでセキュアかつコンプライアンス準拠**。プラットフォームはデフォルトでセキュアであるべきであり、組織が定義したルールや標準に基づいて、コンプライアンスの確保や各種検証を行うための機能を提供する必要があります。
ビジネスにおけるセキュリティ、ガバナンス、およびコンプライアンスに関する組織要件はプラットフォームにあらかじめ組み込まれているべきです。これにより、組織要件の一貫した適用を実現しつつ、ユーザーの認知的負荷を軽減できます。
...
これは、プラットフォームが方向性を見失う最も典型的なパターンのひとつであり、プラットフォームを顧客向け製品として捉え、その成功がユーザーや製品の成功と結び付けて考えることの重要性を示しています。
したがって、プラットフォームチームがアプリチームやその他のユーザーと連携し、プラットフォームの機能やユーザー体験について優先順位付け、計画、実装、そして改善を繰り返していくことが不可欠です。
ユーザーからのフィードバックを得ないまま機能やユーザー体験を提供したり、トップダウンで導入を強制したりするプラットフォームチームは、ユーザーの抵抗や不満を招き、プラットフォームが本来提供すべき価値の多くを損なうことになるでしょう。
...
これは、ユーザーによるセルフサービスを可能にし、手動のステップや必要な人的介入を減らし、セキュリティとコンプライアンスを保証するポリシーを導入することで、しばしば達成されます。
