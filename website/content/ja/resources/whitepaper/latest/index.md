...
プラットフォームは、アプリケーション開発者、データサイエンティスト、インフォメーションワーカーなど、内部顧客の作業を促進し、加速するために、基盤となる能力、フレームワーク、エクスペリエンスを収集・整理して提供します。
最も重要な点として、プラットフォームは、迅速な製品リリース、インフラストラクチャ間の移植性、そしてテクノロジー資産全体にわたる開発者の生産性向上という、クラウドネイティブの価値を企業が実現するのを支援します。
プラットフォームは、各ビジネスに固有でありながら社内のチーム間で共通する要素を体系化することでこれを実現し、ソフトウェアチームが活用できる内部的な規模の経済を生み出します。
...
DevOpsプラクティスの登場は自律性とプロセスの改善をもたらしましたが、同時に、サポートサービスの責任をより多くプロダクトチームに移すという効果も生じました。その結果、プロダクトチームはインフラに関する問題により多くの時間と認知的エネルギーを費やすことを余儀なくされ、組織にとって価値のあるものを生み出すための時間が減少してしまいました。
さらに、チーム間で運用業務が重複することで、実装が肥大化し、責任の所在が不明確になるため、リスクが高まります。
...
1. **暫定段階 \-** 機能は、必要に迫られて臨時スタッフやボランティアによって構築されるため、導入状況が不安定になり、その場しのぎの運用や測定が行われます。

1. **運用段階 \-** 専任の予算配分されたチームが共通機能を（多くの場合事後対応的に）提供し、導入は外部からの指示やインセンティブによって推進され、運用は一元的に追跡されます。

1. **拡張段階 \-** プラットフォームは「製品」として扱われ、顧客価値に基づいた投資が行われ、プロダクト／UX担当者が配置されます。その結果、ユーザーはプラットフォームの本質的な価値（本質的な引力）を理由にそれを選択するようになります。

1. **最適化段階 \-** プラットフォームは組織全体の効率化に焦点を当てた「実現されたエコシステム」となり、中核となるメンテナーはスペシャリストが機能を拡張できるよう支援することを優先し、導入は参加型で行われます。
...
これを防ぐには、プラットフォームチームは最初からプロダクトマネージャーをチームに加え、ロードマップを共有し、フィードバックを集め、プラットフォームユーザーのニーズを全般的に理解し、それを思い浮かべる必要があります。
...
とはいえ、こうしたチームは大切にすべきです。アーリーアダプターと関わり、彼らのフィードバックを取り入れ、当事者意識を持たせることができれば、最終的には、後発の採用者に対してプラットフォームを推進し、普及させる手助けをしてくれるチームが育つでしょう。
...
これを軽減するには、プラットフォームチームは、プロダクトチームやバリューストリームチームに直接的な影響や、両者が関係していることを示す必要があります（前の2つの段落を参照）。プラットフォームチームは、顧客に価値を提供するプロダクトチームの戦略的パートナーとして、その存在をアピールします。
次の章では、その方法について解説します。
...
内部プラットフォームの究極的な目的は、顧客にビジネス価値をより早く提供することであるため、自社の製品や機能リリースに対する影響を測定することで、プラットフォームの目的が達成されていることが証明されます。
